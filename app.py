...
def build_search_tip(df: pd.DataFrame) -> str:
    ...
    if "ml_signal_1m" in df.columns:
        suggestions += ["model_leans_up", "model_leans_down", "model_uncertain"]
...
st.caption(
    "Dividend payouts are not included in this analysis; all insights are based purely "
    "on stock-price movements and historical market patterns."
)
...
# 👉 NEW: Human Advisor Handoff Section
st.sidebar.subheader("👤 Need Human Assistance?")

st.sidebar.write(
    "Have you made up your mind? If you would like to discuss this with a "
    "human advisor, you can proceed below."
)

if st.sidebar.button("📞 Proceed to human advisor"):
    st.sidebar.success(
        "A human advisor can assist you further.\n\n"
        "Please contact your relationship manager, visit your nearest branch, "
        "or call the official K-Trade helpline for personalised investment guidance."
    )
...
# 👉 NEW: Disclaimer block on the left pane
st.sidebar.subheader("⚠️ Disclaimer")

st.sidebar.caption(
    "This is an **AI-based educational tool** built on historical PSX/KSE-100 data. "
    "Outputs are generated by analytics and machine learning models and are **not** "
    "personalized investment advice. Please consult a licensed financial advisor "
...
st.sidebar.caption(
    "Tell us about your experience using this PSX Stock Advisor Chatbot."
)
...
if st.sidebar.button("Submit Feedback"):
    ...
    # (Optional) You can save this later to Google Sheets or a database
    # For now, we only display a thank-you message.
