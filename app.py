...
def render_left_pane():
    ...
    st.write(
        "This tool provides educational insights only, using historical PSX/KSE-100 data. "
        "It does not provide investment advice, recommendations, or forecasts."
    )
    st.write(
        "- AI-generated text may be incomplete or inaccurate.\n"
        "- AI uses historical patterns and does not predict the future.\n"
        "- No personal information is stored or shared."
    )

    profile = st.session_state.get("risk_profile", None)
    if profile:
        st.markdown(f"**Your learning profile:** {profile}")
...
def main():
    ...
    st.markdown(
        ...
        "This is an AI-based educational tool using historical PSX/KSE-100 data. "
    ...
    with right_col:
        ...
        with st.expander("Show full stock card"):
            st.write(row.get("stock_card", ""))
        ...
        # Blue progress bar representing risk on a 0-1 scale
        st.progress(risk_score)

        # Basic numeric snapshot
        ...
        st.write(f"Trend tag: `{row.get('trend', 'N/A')}`, Liquidity: `{row.get('liquidity', 'N/A')}`")
        ...
        user_q = st.text_input("Ask a question about this stock…", key="stock_question")

        if st.button("Send question"):
            question = user_q.strip()
            if question:
                answer = answer_stock_question(row, profile, question)
